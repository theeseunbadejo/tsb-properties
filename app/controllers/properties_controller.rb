class PropertiesController < ApplicationController
  ...
  def show
    ...
    @agent_properties = Property.where(account_id: @agent.id).where.not(id: @property.id) if @agent && @property
  end
  ...
  private
    ...
    def set_property
      @property = Property.find_by_uuid(params[:uuid])
    end
    ...
    def property_params
      params.require(:property).permit(:name, :address, :price, :integer, :rooms, :bathrooms, :parking_spaces, :for_sale, :status, :available_date, :details, :photo_cache, :photo)
    end
end
